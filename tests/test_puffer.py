...
async def test_fehlt_es_im_menue_wird_die_kennung_direkt_gelesen(hass, entry, menu_xml):
    """Wie beim älteren Funktionsblock "Puffer", der es nicht in jedem Menübaum nennt."""
    hass.session.menu = ohne_volumen_im_menue(menu_xml)
    coordinator, by_name = await setup_integration(hass, entry)
    assert coordinator.discovered_uris["puffer_volumen"] == VOLUMEN_URI
    assert "puffer_volumen" in coordinator.ueber_kennung
    assert coordinator.puffer_volumen == 825
    assert "Puffer effektives Volumen" in by_name


async def test_kennt_die_anlage_es_nicht_gibt_es_kein_volumen(hass, entry):
    """Kein Platzhalter mit "-" und kein Energieinhalt - und nichts im Variablensatz."""
    hass.session.menu = ohne_volumen_im_menue(hass.session.menu)
    hass.session.unbekannt.add(VOLUMEN_URI)
    coordinator, by_name = await setup_integration(hass, entry)
...
async def test_energieinhalt_aus_volumen_und_fuehlern(hass, entry, menu_xml):
    ...
    assert energie.extra_state_attributes == {"volumen_liter": 825, "ab_temperatur": 30.0}
...
async def test_ladepumpe_bei_dezentraler_ladung(hass, entry, menu_xml):
    ...
    assert by_name["Puffer Ladepumpe"].native_value == "Heizbetrieb"


async def test_zweiter_puffer_mit_fuehlern_volumen_und_pumpe(hass, entry, menu_xml):
    """Nur Fühler, effektives Volumen und Ladepumpe - kein Ladezustand, kein Energieinhalt."""
...
async def test_alter_puffer_nennt_seine_fuehler_oben_und_unten(hass, entry, menu_xml):
    ...
    assert coordinator.discovered_uris["puffer_volumen"] == "/120/10251/0/0/12499"
